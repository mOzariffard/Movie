import { Client , Databases , Query , ID } from "appwrite";

const PROJECT_ID=import.meta.env.local.VITE_APPWRITE_PROJECT_ID;
const DATABASE_ID=import.meta.env.local.VITE_APPWRITE_DATABASE_ID;
const COLLECTION_ID=import.meta.env.local.VTTE_APPWITE_COLLECTION_ID;


const client = new Client()
    .setEndpoint('http://appwrite.io/v1')
    .setProject(PROJECT_ID)

    const database = new Databases(client);

export const updateSearchCount= async(searchTerm , movie) => {
    try{
        const result = await database.listDocuments(DATABASE_ID, COLLECTION_ID ,[Query.equal('searchTerm', searchTerm)]);

        if(result.documents.length >0){
            const doc=result.documents[0];
        await database.updateDocument(DATABASE_ID, COLLECTION_ID, doc.$id ,{count: doc.count +1});
        }else{
            await database.createDocument(DATABASE_ID,COLLECTION_ID,ID.unique(),{searchTerm, count:1 , movie_id: movie.id} oster_url=`https://image.tmdb.org/t/p/w500/${poster_path}` );
        }
    }catch(error){
        console.error('Error updating search count:', error);
    }
};